-- Log-scaled weighted composite scoring function
-- Normalizes signals using logarithmic scaling for better differentiation
...
DECLARE
  ...
  v_min_score NUMERIC;
  v_max_score NUMERIC;
  -- Dataset maximums for normalization
  max_funding NUMERIC;
  max_patents NUMERIC;
...
BEGIN
  ...
  SET composite_score = ROUND(
    (
      -- Funding: 30% weight, log-scaled
      0.30 * CASE 
        WHEN COALESCE(t.total_funding_eur, 0) > 0 
        THEN LEAST(2, (LOG(t.total_funding_eur + 1) / LOG(max_funding + 1)) * 2)
        ELSE 0 
      END +
      -- Patents: 25% weight, log-scaled  
      0.25 * CASE 
        WHEN COALESCE(t.total_patents, 0) > 0 
        THEN LEAST(2, (LOG(t.total_patents + 1) / LOG(max_patents + 1)) * 2)
        ELSE 0 
      END +
      -- Employees: 25% weight, log-scaled
      0.25 * CASE 
        WHEN COALESCE(t.total_employees, 0) > 0 
        THEN LEAST(2, (LOG(t.total_employees + 1) / LOG(max_employees + 1)) * 2)
        ELSE 0 
      END +
      -- Company count: 20% weight, log-scaled
      0.20 * CASE 
        WHEN COALESCE(t.dealroom_company_count, 0) > 0 
        THEN LEAST(2, (LOG(t.dealroom_company_count + 1) / LOG(max_companies + 1)) * 2)
        ELSE 0 
      END
    ), 2
  )
...
-- Also update the trigger to use log-scaled approach
...
DECLARE
  ...
  max_companies NUMERIC;
BEGIN
  -- Get dataset maximums for relative scoring
  SELECT 
    GREATEST(MAX(total_funding_eur), 1),
    GREATEST(MAX(total_patents), 1),
    GREATEST(MAX(total_employees), 1),
  ...
  -- Log-scaled weighted composite (0-2 range)
  NEW.composite_score := ROUND(
    (
      0.30 * CASE 
        WHEN COALESCE(NEW.total_funding_eur, 0) > 0 
        THEN LEAST(2, (LOG(NEW.total_funding_eur + 1) / LOG(max_funding + 1)) * 2)
        ELSE 0 
      END +
      0.25 * CASE 
        WHEN COALESCE(NEW.total_patents, 0) > 0 
        THEN LEAST(2, (LOG(NEW.total_patents + 1) / LOG(max_patents + 1)) * 2)
        ELSE 0 
      END +
      0.25 * CASE 
        WHEN COALESCE(NEW.total_employees, 0) > 0 
        THEN LEAST(2, (LOG(NEW.total_employees + 1) / LOG(max_employees + 1)) * 2)
        ELSE 0 
      END +
      0.20 * CASE 
        WHEN COALESCE(NEW.dealroom_company_count, 0) > 0 
        THEN LEAST(2, (LOG(NEW.dealroom_company_count + 1) / LOG(max_companies + 1)) * 2)
        ELSE 0 
      END
    ), 2
  );
