...
import { Radar, Grid3X3, Compass, TrendingUp, FileText, Activity, Zap, Database, RefreshCw, ArrowRight, Sparkles, Clock } from "lucide-react";
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between">
                <div>
                  <CardTitle>Top Performing Technologies</CardTitle>
            ...
            <Card>
              ...
              <CardContent>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {(Object.entries(stats.quadrantCounts) as [string, number][]).map(([quadrant, count]) => (
                    <div key={quadrant} className="p-4 rounded-lg bg-muted/50 text-center">
                ...
                <div className="mt-4 pt-4 border-t border-border">
                  <div className="flex flex-wrap gap-3 justify-center">
                    {(Object.entries(stats.ringCounts) as [string, number][]).map(([ring, count]) => (
                      <Badge key={ring} variant="outline" className="gap-1">
                        <span className="font-semibold">{count}</span> {ring}
                      </Badge>
                    ))}
                  </div>
                </div>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                {trendingTechnologies.map((tech) => (
                  <div key={tech.id} className="flex items-center gap-4 p-3 rounded-lg bg-muted/30">
                    ...
                    <div className="text-right shrink-0">
                      <p className="font-mono font-bold">{tech.compositeScore.toFixed(1)}</p>
                    </div>
                  </div>
                ))}
              </CardContent>
                ...
                {aiInsights.map((item, i) => (
                  <div key={i} className="p-3 rounded-lg bg-card border border-border">
                    <div className="flex gap-2">
                      <div className={`w-1 rounded shrink-0 ${item.severity === "high" ? "bg-emerald-500" : item.severity === "medium" ? "bg-amber-500" : "bg-sky-500"}`} />
              ...
              <CardContent className="space-y-3">
                {recentActivity.map((item, i) => (
                  <div key={i} className="flex items-start gap-3">
                    <div className={`w-2 h-2 rounded-full mt-2 shrink-0 ${item.type === "data" ? "bg-sky-500" : item.type === "score" ? "bg-emerald-500" : "bg-violet-500"}`} />
                    <div className="flex-1 min-w-0">
              ...
              <CardContent className="space-y-2">
                {[
                  ...
                  <div key={source.name} className="flex items-center justify-between text-sm">
                    ...
                    <Badge variant="outline" className={source.status === "healthy" ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/30" : "bg-amber-500/10 text-amber-400 border-amber-500/30"}>
                      {source.status}
                    </Badge>
                  </div>
                ))}
