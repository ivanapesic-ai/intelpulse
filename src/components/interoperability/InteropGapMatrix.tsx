    ...
    <TableCell className="text-center">
      <TooltipProvider delayDuration={200}>
        <Tooltip>
          ...
          <TooltipContent>
            <p className="text-xs">{count} {label}</p>
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>
    </TableCell>
...
function ReadinessScore({ signals }: { signals: number[] }) {
  ...
  const color =
    withData >= 4 ? "bg-emerald-500/10 text-emerald-500 border-emerald-500/30" :
    withData >= 2 ? "bg-yellow-500/10 text-yellow-500 border-yellow-500/30" :
  ...
  const rows = useMemo(() => {
    return keywords
      .map((kw) => {
        ...
        const signalCount = [standards, charin, cordis, news].filter((s) => s > 0).length;
        return { keyword: kw, standards, charin, cordis, news, signalCount };
      })
      .sort((a, b) => a.signalCount - b.signalCount);
  }, [keywords, standardsByKeyword, charinByKeyword, cordisByKeyword, newsByKeyword]);
              ...
              <TableBody>
                {rows.map((row) => (
                  <TableRow key={row.keyword.id} className={row.signalCount <= 1 ? "bg-destructive/5" : ""}>
                    ...
                    <ReadinessScore signals={[row.standards, row.charin, row.cordis, row.news]} />
                  </TableRow>
                ))}
              </TableBody>
            </Table>
